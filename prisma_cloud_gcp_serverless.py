"""
A GCP cloud function to forward Prisma Cloud webhook messages to syslog in LEEFJson format
"""
#
...
def main(request):
    ...
    for alert in range(len(request_json)):
        m.body = "{event_attributes}".format(event_attributes=lib.LEEFJson.parseJson(request_json[alert]))
        syslogger.info(m.body)
